...
2. **Ask the git preferences question** (foreground, while npm runs):
   - Use `AskUserQuestion` with a single question:
     - "How should Claude handle git commits and pushes?" — Options:
       - "Auto-approve both commits and pushes" (recommended for maximum speed)
       - "Auto-approve commits only"
       - "Auto-approve pushes only"
       - "Always ask — I want to review and approve each commit and push" (recommended for maximum control)
   - Map their answer to flags and run the init script via Bash:
     ...
     ```
   - **Do NOT use the `Write` tool for this file.** The script is auto-approved by the bash-permission-checker hook; using `Write` directly would trigger a permission prompt for the `.claude/` folder.
...
**Do NOT ask the user what to do next.** The summary is the final output of the setup phase. Do NOT stop or end the conversation here — when invoked from `/start`, the calling command will continue with the next step. When run standalone, the user can see the next steps and decide on their own.
